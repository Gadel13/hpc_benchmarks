...
struct option longopts[] = {
    ...
    { "overlap_x", 0, NULL, OPT_OVERLAP_X },
    { "overlap_y", 0, NULL, OPT_OVERLAP_Y },
	 ...
	 char **argv)
{
    ...
    for (iter=0; iter < 1; iter++) 
    {
	    ...
	    /* touch all the pages */
	    srand(time(NULL));
	    for (i = 0; i < sz_tile_x * sz_tile_y * sz_element; i += 1) {
		buffer[i] = (char) rand();
	    }
	    ...
	    if ((my_tile[1] == 0) || (nr_files != 1)) off_tile[1] = 0;
	    else off_tile[1] = my_tile[1] * (sz_tile_x - overlap_x);

	    if ((my_tile[0] == 0) || (nr_files != 1)) off_tile[0] = 0;
	    else off_tile[0] = my_tile[0] * (sz_tile_y - overlap_y);

	//#if 0
	    printf("my rank = %d, my coords = (%d,%d), my offset = (%d,%d), my size write/read Mbytes = (%d)\n", 
		   my_rank, my_tile[1], my_tile[0], off_tile[1], off_tile[0], (sz_tile_x * sz_tile_y * sz_element)/(1024*1024));
	//#endif
	    ...
	    //if write/read to own file filename --> filename_<my_rank>
	    if(nr_files != 1 && iter == 0)
	    {
	    	strcat(filename, "_");
	    	char str_rank[3];
	    	itoa(my_rank, str_rank);
	    	strcat(filename, str_rank);
	    }

	    char file_data[99];
	    strcpy(file_data, "DATA_FROM_TESTS_");
	    strcat( file_data, "FILES");
	    char tmp[3];
	    itoa(nr_files, tmp);
	    strcat(file_data, tmp);
	    strcat(file_data, "_PROC");
	    itoa(nr_procs , tmp);
	    strcat(file_data, tmp);
	    strcat(file_data, ".csv");

	    double open_max_time_one;
	    double io_max_time_one;
	    double close_max_time_one;
	    int64_t size =  (int64_t) sz_tile_x * (int64_t) sz_tile_y * (int64_t) nr_tiles_x * (int64_t) nr_tiles_y * (int64_t) sz_element;
	    size /= (int64_t)1000;
	    size *= (int64_t)8;
	    size /= (int64_t)1000000;

	    /* foreach file */
		//printf("Iteration%d myid: %d\n", iter, my_rank);

	    open_max_time_one = 0;
	    io_max_time_one = 0;
	    close_max_time_one = 0;
		...
		MPI_Reduce(&open_time, &open_max_time_one, 1, MPI_DOUBLE, MPI_MAX, 0, cart_comm);
		MPI_Reduce(&io_time, &io_max_time_one, 1, MPI_DOUBLE, MPI_MAX, 0, cart_comm);
		MPI_Reduce(&close_time, &close_max_time_one, 1, MPI_DOUBLE, MPI_MAX, 0, cart_comm);

		if( my_rank == 0 )
		{
			FILE *file;
			double speed_io = (double)size / io_max_time_one;
			file = fopen(file_data, "a");
			if(write_file)
				fprintf(file, "%d, %d, write, %d, %f, %f, %f, %f, %d, %d, %d, %d, %d\n", iter, nr_procs, nr_files, speed_io, open_max_time_one, io_max_time_one, close_max_time_one, sz_tile_x, sz_tile_y, nr_tiles_x, nr_tiles_y, sz_element );//iteration, numproc, operation, max_open, max_io, max_close, sz_tile_x, sz_tile_y, nr_ti;es_x, nr_tilex_y, sz_element
			else
				fprintf(file, "%d, %d, read, %d, %f, %f, %f, %f, %d, %d, %d, %d, %d\n", iter, nr_procs, nr_files, speed_io, open_max_time_one, io_max_time_one, close_max_time_one, sz_tile_x, sz_tile_y, nr_tiles_x, nr_tiles_y, sz_element );//iteration, numproc, operation, max_open, max_io, max_close, sz_tile_x, sz_tile_y, nr_ti;es_x, nr_tilex_y, sz_element
...
void pass_parameters(void)
{
	...
	parameters[5] = nr_files;
	parameters[6] = overlap_x;
	parameters[7] = overlap_y;
	parameters[8] = sz_coll_buf;
	parameters[9] = sz_indep_buf;
	parameters[10] = collective;
	parameters[11] = header_bytes;
	parameters[12] = write_file;
	if (filename) parameters[13] = strlen(filename) + 1;
	else parameters[13] = 0;
    }

    MPI_Bcast(parameters, 14, MPI_INT, 0, MPI_COMM_WORLD);
    
    /* everyone but process 0 needs to allocate a filename */
    if (my_rank != 0 && parameters[13]) filename = malloc(parameters[32]);
    ...
    MPI_Bcast(filename, parameters[13], MPI_CHAR, 0, MPI_COMM_WORLD);
	...
	nr_files = parameters[5];
	overlap_x = parameters[6];
	overlap_y = parameters[7];
	sz_coll_buf = parameters[8];
	sz_indep_buf = parameters[9];
	collective = parameters[10];
	header_bytes = parameters[11];
	write_file = parameters[12];
    }
}
		    ...
		    int print)
{
	 ...
	 */
	printf("total size (bits)%ld\n", total_size*8);
	total_size = total_size / (int64_t) 1000000;
	if (max_time != 0.0)
	...
	if (write_file)
	    printf("Write Bandwidth = %.6f Gbits/sec\n", (float) io_bw);
	else
    	    printf("Read Bandwidth = %.6f Gbits/sec\n", (float) io_bw);
    }
}
